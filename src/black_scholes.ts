...
function probability(price: number, target: number, days: number, volatility: number): { pbelow: number; pabove: number; } {
    const p = price;
    const q = target;
    const t = days / 365;
    const v = volatility;
 ...
 */
export function greeks(call: boolean, S: number, K: number, r: number, T: number, v: number): { delta: number; gamma: number; theta: number; vega: number; rho: number; price: number; } {
