...
export class PricesUpdateJob extends CronJob {
  task: TradeBot;
  ...
  public async process(): Promise<JobResult> {
    ...
    await this.task.portfolio
      ...
      .then(async () => this.task.updateCurrencyContracts())
      .then(async () => this.task.updateSettingsContracts(this.task.portfolio.settings))
      .then(async () => this.task.updatePositionsContracts())
      .then(() => {
        success = true;
      })
      .catch((error) => {
    ...
    const result: JobResult = {
      success,
      message: success ? "PricesUpdate job completed" : "PricesUpdate job failed",
