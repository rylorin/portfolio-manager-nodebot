...
export class OptionChainMaintenanceJob extends CronJob {
  task: TradeBot;
  ...
  public async process(): Promise<JobResult> {
    ...
    await this.task.portfolio
      ...
      .then(async () => this.task.expireOptions())
      .then(async () => this.task.addOptionsChains())
      .then(() => {
        success = true;
      })
      .catch((error) => {
    ...
    const result: JobResult = {
      success,
      message: success ? "OptionChainMaintenance job completed" : "OptionChainMaintenance job failed",
